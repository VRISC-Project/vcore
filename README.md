...
* 带有cmake插件的编辑器/IDE可直接使用

### 运行

```bash
sudo ./build/vrisc/vrisc -m 1048576 -c 1 -b ./boot/boot.bin
```
...
> 目前中断和io还没有提供给外部访问的库，
> 可以通过自己写程序访问ioctl.c和intctl.c中创建的共享内存来访问。
...
只有16个通用寄存器可以在汇编指令中使用，其它寄存器只能间接访问或者无法访问。
...
> 接下来的新增指令将写在[指令集文档](docs/instruction_set.md)中。
